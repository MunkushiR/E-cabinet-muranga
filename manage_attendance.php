		...
		<div class="row form-group">
			<div class="col-md-4">
				...
				<select id="employee_id" class="borwser-default select2">
					<option value=""></option>
					<?php 
					$employee = $conn->query("SELECT *,concat(lastname,', ',firstname) as ename FROM employees order by concat(lastname,', ',firstname) asc");
					while($row = $employee->fetch_assoc()):
					?>
						<option value="<?php echo $row['id'] ?>"><?php echo $row['ename'] . ' | '. $row['personal_number'] ?></option>
					<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-3">
